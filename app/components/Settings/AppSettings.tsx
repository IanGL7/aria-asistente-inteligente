...
const AppSettings: React.FC<AppSettingsProps> = ({
  ...
  <h2 className="text-lg font-semibold border-b border-gray-200 pb-2">
    Ajustes del dispositivo Aria
  </h2>

  {skipDeviceRegistration && (
    <div className="flex items-center text-purple-500 text-xs gap-2">
      En este entorno de pruebas no es necesario registrar tu dispositivo
      Aria (NEXT_PUBLIC_SKIP_DEVICE_REGISTRATION = True).
    </div>
  )}
  ...
  <div className="flex flex-col gap-6">
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="flex flex-row items-center gap-2 mt-2">
        <Input
          ...
          placeholder={
            isConnected
              ? "Código registrado"
              : "Ingresa el código de tu dispositivo Aria"
          }
          maxLength={100}
        />
        <Button
          ...
          disabled={isConnected || skipDeviceRegistration}
          onClick={async () => {
            const result = await connectUserToDevice(
      ...
      <p className="text-xs text-gray-400">
        {isConnected ? (
          ...
          "Ingresa el código de tu dispositivo Aria para vincularlo a este perfil."
        )}
      </p>
    </div>
    ...
    {/* Volumen del dispositivo (solo si está conectado) */}
    {isConnected && (
      <div className="flex flex-col gap-4 mt-6">
        <Label className="text-sm font-medium text-gray-700">
          Volumen del dispositivo Aria
        </Label>
